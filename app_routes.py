...
from app_core import DATABASE, get_current_competition_id, query_db
from genereer_rondes import BuildNextRound, RefreshPlayersResults, SaveResultsToPlayers
...
def register_routes(app):
    ...
    def get_competition_player_groups(competition_id):
        rows = query_db(
            """
            SELECT
                p.Id,
                p.Name,
                COALESCE(cpg.GroupNumber, p.GroupNumber) AS GroupNumber,
                CASE WHEN cp.PlayerId IS NULL THEN 0 ELSE 1 END AS InCompetition
            FROM Players p
            LEFT JOIN CompetitionPlayers cp
                ON cp.PlayerId = p.Id
               AND cp.CompetitionId = ?
            LEFT JOIN CompetitionPlayerGroups cpg
                ON cpg.PlayerId = p.Id
               AND cpg.CompetitionId = ?
            WHERE p.Active = 1
            ORDER BY p.Name ASC
            """,
        ...
        return [
            {
                ...
                "name": row[1],
                "group_number": row[2],
                "in_competition": bool(row[3]),
            }
            for row in rows
        ]

    def get_competition_group_numbers(competition_id):
        rows = query_db(
            """
            SELECT GroupNumber
            FROM CompetitionGroupPoints cgp
            INNER JOIN CompetitionSettings cs
                ON cgp.CompetitionSettingsId = cs.Id
            WHERE cs.CompetitionId = ?
        ...
        if not rows:
            rows = query_db(
                """
                SELECT DISTINCT COALESCE(cpg.GroupNumber, p.GroupNumber)
    ...
    def get_competition_settings_data(competition_id):
        player_groups = get_competition_player_groups(competition_id)
    ...
    def spelers():
        competition_id = get_current_competition_id()
        groups = get_competition_group_numbers(competition_id)
        if not groups:
            groups = [1]
        rows = query_db(
            """
        ...
        players = [
            {"id": r[0], "name": r[1], "rating": r[2], "group_number": r[3]}
            for r in rows
        ]
        available_rows = query_db(
            """
            SELECT p.Id, p.Name
            FROM Players p
            LEFT JOIN CompetitionPlayers cp
        ...
        available_players = [{"id": r[0], "name": r[1]} for r in available_rows]
    ...
    @app.route("/spelers/add-existing", methods=["POST"])
    def add_existing_player_to_competition():
        competition_id = get_current_competition_id()
        ...
        cur.execute(
            """
            INSERT OR IGNORE INTO CompetitionPlayers
        ...
        cur.execute(
            """
            INSERT INTO CompetitionPlayerGroups
                (CompetitionId, PlayerId, GroupNumber)
            VALUES (?, ?, ?)
            ON CONFLICT(CompetitionId, PlayerId) DO UPDATE SET
                GroupNumber = excluded.GroupNumber,
                Last_Update = CURRENT_TIMESTAMP
            """,
            (competition_id, player_id, group_number),
        )
        cur.execute(
            """
            INSERT OR IGNORE INTO Present
                (PlayerId, RoundId, Present, ReasonAbsentId)
            SELECT ?, Id, 1, NULL
            FROM Rounds
            WHERE CompetitionId = ?
            """,
    ...
    @app.route("/spelers/<player_id>/group", methods=["POST"])
    def update_player_competition_group(player_id):
        competition_id = get_current_competition_id()
        group_number = request.form.get("group_number", type=int)
        if group_number not in get_competition_group_numbers(competition_id):
            abort(400)
        ...
        cur.execute(
            ...
            WHERE CompetitionId = ? AND PlayerId = ?
            """,
            (competition_id, player_id),
        )
        if not cur.fetchone():
    ...
    @app.route("/spelers/add", methods=["GET", "POST"])
    def add_player():
        competition_id = get_current_competition_id()
        groups = get_competition_group_numbers(competition_id)
        if not groups:
        ...
        player = {
            "name": "",
            "rating": "",
            "group_number": groups[0],
        }

        if request.method == "POST":
            name = request.form.get("name", "").strip()
            rating_value = request.form.get("rating", "").strip()
            group_value = request.form.get("group_number", "").strip()
            player = {
                "name": name,
                "rating": rating_value,
            ...
            try:
                group_number = int(group_value)
            except ValueError:
                return render_template(
                    "add_player.html",
                    player=player,
                    groups=groups,
                    error="Groep moet een geheel getal zijn.",
                ), 400

            conn = sqlite3.connect(DATABASE)
            cur = conn.cursor()
            next_id_row = cur.execute(
                """
                SELECT COALESCE(MAX(CAST(Id AS INTEGER)), 0) + 1
                FROM Players
                WHERE Id GLOB '[0-9]*' AND Id != '999'
                """
            ).fetchone()
            player_id = str(next_id_row[0])
            cur.execute(
                """
                INSERT INTO Players
                    (Id, Name, GroupNumber, Active, Rating, Type, Last_Update)
                VALUES (?, ?, ?, 1, ?, NULL, CURRENT_TIMESTAMP)
                """,
                (player_id, name, group_number, rating),
            )
            cur.execute(
                """
                INSERT INTO CompetitionPlayers
                    (CompetitionId, PlayerId)
                VALUES (?, ?)
                """,
                (competition_id, player_id),
            ...
            round_rows = cur.execute(
                """
                SELECT Id
                FROM Rounds
                WHERE CompetitionId = ? AND Played = 0
                """,
                (competition_id,),
            ).fetchall()
            cur.executemany(
                ...
                VALUES (?, ?, 1, NULL)
                """,
                [(player_id, round_id) for (round_id,) in round_rows],
            )
            conn.commit()
            conn.close()
            return redirect(url_for("spelers"))

        return render_template("add_player.html", player=player, groups=groups)
    ...
    def delete_player(player_id):
        ...
        cur.execute(
            """
            DELETE FROM CompetitionPlayerGroups
            WHERE CompetitionId = ? AND PlayerId = ?
            """,
            (competition_id, player_id),
        )
        cur.execute(
            """
            DELETE FROM CompetitionPlayers
            WHERE CompetitionId = ? AND PlayerId = ?
            """,
            (competition_id, player_id),
        )
    ...
    def player_overview():
        ...
        rows = query_db("""
            ...
            WHERE cp.CompetitionId = ?
              ...
              AND pr.RoundId = ?
            ORDER BY p.Name ASC
        """, (competition_id, active_round_id,))
        players = [
            {
                "id": r[0],
                "name": r[1],
    ...
    def genereer_ronde():
        ...
        pairings = [
            {"player1": r[0], "player2": r[1], "round": r[2], "group": r[3]}
            for r in rows
        ]
    ...
    def ranking():    
        competition_id = get_current_competition_id()
        RefreshPlayersResults(DATABASE)
        settings_type = get_competition_settings_data(competition_id)["settings_type"]
        date = query_db(
        ...
        if settings_type == "percentage":
            rows = query_db("""
                SELECT
                    ...
                    SUM(
                        CASE
                            WHEN r.Resultstype = 1 THEN 1.0
                            WHEN r.Resultstype = 2 THEN 0.5
                            ELSE 0
                        END
                    ) AS MatchScore,
                    SUM(CASE WHEN r.Resultstype IN (1, 2, 3) THEN 1 ELSE 0 END) AS MatchesPlayed,
                    CASE
                        WHEN SUM(CASE WHEN r.Resultstype IN (1, 2, 3) THEN 1 ELSE 0 END) > 0
                        THEN SUM(
                            CASE
                                WHEN r.Resultstype = 1 THEN 1.0
                                WHEN r.Resultstype = 2 THEN 0.5
                                ELSE 0
                            END
                        ) / SUM(CASE WHEN r.Resultstype IN (1, 2, 3) THEN 1 ELSE 0 END)
                        ELSE 0
                ...
                LEFT JOIN Results r ON pr.ResultId = r.Id
                WHERE p.Active = 1
                GROUP BY p.Id, p.Name, COALESCE(cpg.GroupNumber, p.GroupNumber)
                ORDER BY GroupNumber ASC, MatchPercentage DESC, MatchScore DESC, MatchesPlayed DESC
            """, (competition_id, competition_id,))
            players = [
                {
                    "id": r[0],
                    ...
                    "GroupNumber": r[2],
                    "MatchScore": r[3] or 0,
                    "MatchesPlayed": r[4] or 0,
                    "MatchPercentage": r[5] or 0,
                }
                for r in rows
            ]
        else:
            rows = query_db("""
                ...
                WHERE p.Active = 1
                GROUP BY p.Id, p.Name, COALESCE(cpg.GroupNumber, p.GroupNumber)
                ORDER BY GroupNumber ASC, TotalPoints DESC
            """, (competition_id, competition_id,))
    ...
    def player_results(player_id):
        ...
        rows = query_db("""
            SELECT
                b.Id as OpponentId,
                b.Name as OpponentName,
                c.Name as ResultsName,
                c.Points,
                c.Resultstype,
                d.Date,
                d.Period
        ...
        results = [
            {
                ...
                "result_id": r[2],
                "Points": r[3],
                "MatchScore": {1: 1, 2: 0.5, 3: 0}.get(r[4]),
                "Date": r[5],
                "Period": r[6],
            }
            for r in rows
    ...
    def competition_settings():
        ...
        player_groups = configuration.get("player_groups", [])
        ...
        active_player_rows = query_db(
            "SELECT Id FROM Players WHERE Active = 1"
        )
        active_player_ids = {str(row[0]) for row in active_player_rows}
        allowed_group_numbers = set(seen_group_numbers)
        if not isinstance(player_groups, list):
            return render_error("De spelergroepen bevatten ongeldige gegevens")

        validated_player_groups = []
        seen_player_ids = set()
        for player_group in player_groups:
            if not isinstance(player_group, dict):
                return render_error("Een spelergroep bevat ongeldige gegevens")

            player_id = str(player_group.get("player_id", "")).strip()
            in_competition = bool(player_group.get("in_competition"))
            group_number = player_group.get("group_number")
            if player_id not in active_player_ids:
                return render_error("Een speler in de spelergroepen is ongeldig")
            if player_id in seen_player_ids:
                return render_error("Een speler komt meer dan een keer voor bij spelergroepen")
            if (
                in_competition
                and (
                not isinstance(group_number, int)
                or isinstance(group_number, bool)
                or group_number not in allowed_group_numbers
                )
            ):
                return render_error("Iedere speler moet in een bestaande groep zitten")

            seen_player_ids.add(player_id)
            validated_player_groups.append((player_id, group_number, in_competition))
        if not any(in_competition for _, _, in_competition in validated_player_groups):
            return render_error("Selecteer minimaal één speler voor deze competitie")
        ...
        try:
            ...
            cur.execute(
                "DELETE FROM CompetitionPlayers WHERE CompetitionId = ?",
                (competition_id,),
            )
            cur.executemany(
                """
                ...
                [
                    (competition_id, player_id)
                    for player_id, _, in_competition in validated_player_groups
                    if in_competition
                ],
            )
            cur.execute(
                ...
                INSERT OR IGNORE INTO Present
                    (PlayerId, RoundId, Present, ReasonAbsentId)
                SELECT cp.PlayerId, r.Id, 1, NULL
                FROM CompetitionPlayers cp
                INNER JOIN Rounds r ON r.CompetitionId = cp.CompetitionId
                WHERE cp.CompetitionId = ?
                """,
                (competition_id,),
            )
            cur.execute(
                "DELETE FROM CompetitionPlayerGroups WHERE CompetitionId = ?",
                (competition_id,),
            )
            cur.executemany(
                """
                ...
                [
                    (competition_id, player_id, group_number)
                    for player_id, group_number, in_competition in validated_player_groups
                    if in_competition
                ],
            )
            conn.commit()
    ...
    def round_editor():
        ...
        if selected_round is None:
            group_numbers = get_competition_group_numbers(competition_id)
            return render_template(
                "round_editor.html",
                round_id=None,
                rounds=rounds,
    ...
    def round_editor_add_pairing():
        ...
        if group_number not in get_competition_group_numbers(competition_id):
            return jsonify({"success": False, "error": "De groep bestaat niet in deze competitie"}), 400
    ...
    def round_editor_swap_players():
        ...
        cur.execute(
            """
            SELECT COALESCE(cpg.GroupNumber, p.GroupNumber)
            FROM Players p
            LEFT JOIN CompetitionPlayerGroups cpg
                ON cpg.PlayerId = p.Id
               AND cpg.CompetitionId = ?
            WHERE p.Id = ?
            """,
            (competition_id, str(player_a_id)),
        )
        ...
        cur.execute(
            ...
            WHERE p.Id = ?
            """,
            (competition_id, str(player_b_id)),
        )
